...
function checkPasswords() {
    ...
    if (password === confirmPassword && password.length > 0) {
        ...
        confirmPasswordMsg.html('<p class="errormsg">كلمات المرور غير متطابقة  </p>');
    }
}
 ...
 confirmPswrdInp.on('input', checkPasswords);
    ...
    $('#submit-button').on('click', function() {
        ...
        // Validate first name
        if (fName.length < 3) {
            $('#fname-err').html('<p class="errormsg">يرجى إدخال اسم أول متطابق  </p>');
        ...
        // Validate address
        if (address.length < 7) {
            $('#address-err').html('<p class="errormsg">يرجى إدخال عنوان متطابق  </p>');
        ...
        // Validate city
        if (city.length < 1) {
            $('#city-err').html('<p class="errormsg">يرجى إدخال المنطقة  </p>');
        ...
        if (!(mobile.length === 9 || mobile.length === 10)) {
            $('#mobile-err').html('<p class="errormsg">يرجى إدخال رقم هاتف متطابق علي النحو الأتى : 5xxxxxxxx  </p>');
        ...
        if (!emailRegex.test(email)) {
            $('#email-err').html('<p class="errormsg">يرجى إدخال بريد إلكتروني صالح  </p>');
        ...
        if (passwordInput.value !== cpasswordInput.value) {
            $('#confirm-password-msg').html('<p class="errormsg">كلمات المرور غير متطابقة  </p>');
        ...
        if (!passwordIsValid) {
            $('#pswrderrr').html('<p class="errormsg">كلمة المرور لا تفي بجميع المتطلبات  </p>');
            return false; // Prevent form submission
        } else {
